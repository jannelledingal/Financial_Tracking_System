...
<div class="space-y-6">
    <div class="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        <div>
            <p class="text-sm uppercase tracking-[0.24em] text-slate-500">Expenses</p>
            <h1 class="mt-2 text-3xl font-semibold text-slate-950">Expense tracker</h1>
            <p class="mt-2 text-sm text-slate-500">Review spending by account and category.</p>
        </div>
        <div class="rounded-full border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-700">Total: ₱{{ number_format($totalExpense, 0) }}</div>
    </div>
            ...
            <tbody class="divide-y divide-slate-200 bg-white text-slate-700">
                @forelse($transactions as $transaction)
                    <tr>
                        ...
                        <td class="px-6 py-4 text-right font-semibold text-rose-600">-₱{{ number_format(abs($transaction->amount), 0) }}</td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="4" class="px-6 py-12 text-center text-slate-500">No expense records found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
